...
def linear_map(data, new_min=0, new_max=10):
    ...
    # 使用线性映射公式进行转换
    mapped_data = [(new_max - new_min) * (x - old_min) / (old_max - old_min) + new_min for x in data]
...
def calculate_entropy(user_labels, epsilon=1e-10):
    ...
    for i in range(len(user_labels)):
        b.append([])
    for j in range(len(user_labels)):
        for i in b:
            i.append(0)
    for i in range(len(user_labels)):
    ...
    entropys = []
    for i in b:
        label_frequencies = np.array(i)

        # 计算熵
        # 对于零频率标签，使用epsilon替代
        label_frequencies_with_epsilon = np.where(label_frequencies > 0, label_frequencies, epsilon)

        entropy = -np.sum(label_frequencies_with_epsilon * np.log(label_frequencies_with_epsilon))

        # 返回熵和零频率标签的数量
        entropys.append(entropy)
        # zero_count = np.sum(label_frequencies == 0)
    return entropys
...
net_client_local=ResNet18_client_side()
...
net_glob_server = ResNet18_server_side()  # 7 is my numbr of classes
net_server_local = ResNet18_server_side()
...
w_locals_server = []
w_locals_biaoji = [0 for i in range(int(num_users / 100))]
# client idx collector
idx_collect = []
l_epoch_check = False
fed_check = False
# Initialization of net_model_server and net_server (server-side model)
net_model_server = [net_glob_server for i in range(int(num_users / 100))]
...
def train_server(fx_client, y, l_epoch_count, l_epoch, idx, len_batch):
    ...
    net_server = copy.deepcopy(net_model_server[int(idx / 100)]).to(device)
    net_server.train()
    ...
    net_model_server[int(idx / 100)] = copy.deepcopy(net_server)
    ...
    if count1 == len_batch:
        ...
        if l_epoch_count == l_epoch - 1:
            w_locals_biaoji[int(idx / 100)] += 1
            l_epoch_check = True  # to evaluate_server function - to check local epoch has completed or not
            # We store the state of the net_glob_server()
            if w_locals_biaoji[int(idx / 100)] == 100:
...
dataset_train = datasets.FashionMNIST('./data2/fmnist', train=True, download=True, transform=transform)
dataset_test = datasets.FashionMNIST('./data2/fmnist', train=False, download=True, transform=transform)
predict_dataset = datasets.FashionMNIST('./data2/fmnist', train=True, download=True, transform=transform)
...
zhenshizaosheng=[]
with open('fmnist0.1n=100.txt', 'r') as file:
    content = file.read()
...
leibie_final = leibie_entropy#这里先手动定义为leibie_lid，运行时手动切换


noise_clients=[i for i, value in enumerate(leibie_entropy) if value == 0]
clean_clients=[i for i, value in enumerate(leibie_entropy) if value == 1]
...
for client_id in noise_clients:
        ...
        relabel_idx = (-loss).argsort()[:int(len(pred_n)*0.8)]
