...
predict_dataset = datasets.CIFAR10('./data', train=True, download=True, transform=transform_test)
...
zhenshizaosheng=[]
with open('cifar10_n=100_alpha=0.1.txt', 'r') as file:
    content = file.read()
...
# with open('cifar10_85.94_iid.txt', 'r') as file:
#     content = file.read()
# train_noise_label=eval(content)
for i in range(num_users):
    # print(i)
    ...
    for j in dict_users[i]:
        ...
        # if dataset_train.targets[j] != train_noise_label[j]:
        #     zhenshizaosheng.append(j)
        # dataset_train.targets[j] = train_noise_label[j]
...
for client_id in noise_clients:
        ...
        relabel_idx = (-loss).argsort()[:int(len(pred_n)*0.6)]
        # relabel_idx = list(set(np.where(np.max(local_output, axis=1) < 0.5)[0]) & set(relabel_idx))##启用relabel_idx一共需要解除4行注释
        print(relabel_idx)
